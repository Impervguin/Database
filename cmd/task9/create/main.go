...
func CreatePipeline(st *task9.Task9Storage) {
	id, err := st.GetRandomClientId()
	if err != nil {
		log.Println("Error getting random client ID:", err)
		return
	}
	acc, err := task9.CreateRandonAccount(id)
	if err != nil {
		log.Println("Error creating account:", err)
		return
	}
	err = st.CreateAccount(acc)
	if err != nil {
		log.Println("Error creating account:", err)
		return
	}
...
func main() {
	...
	_, err = scheduler.NewJob(gocron.DurationJob(ChangeSeconds), gocron.NewTask(CreatePipeline, storage))
	if err != nil {
		panic(err)
	}
