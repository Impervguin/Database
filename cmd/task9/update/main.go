...
func UpdatePipeline(st *task9.Task9Storage) {
	acc, err := st.GetRandomActiveAccount()
	if err != nil {
...
func main() {
	...
	_, err = scheduler.NewJob(gocron.DurationJob(ChangeSeconds), gocron.NewTask(UpdatePipeline, storage))
	if err != nil {
		panic(err)
	}
